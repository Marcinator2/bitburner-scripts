...
const TAB_DISPLAY_TYPE = { services: "grid", training: "grid", gang: "grid", augments: "block", server: "block", corporation: "block", ipvgo: "block", infiltrate: "block" };
...
export async function main(ns) {
  ...
  // Force RAM allocation for APIs called inside helper functions (scanner may miss them)
  // eslint-disable-next-line no-constant-condition
  if (false) { ns.infiltration.getInfiltrations(); }
...
function renderPanel(ns, panel) {
  ...
  // Infiltrate tab rendering
  if (panel.infiltrate) {
    renderInfiltratePane(ns, panel);
  }
}

function renderInfiltratePane(ns, panel) {
  ...
  if (hasFullApi) {
    ...
    // Fallback: getPossibleLocations() has no difficulty/reward data
...
function buildInfiltratePane(doc) {
  const wrap = doc.createElement("div");
